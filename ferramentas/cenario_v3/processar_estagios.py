...
porque a quantização fixa cores de pixels que ainda vão ser mesclados:
  ...
  3. paleta derivada do estágio 6 da estação, 48–56 cores
...
# A ordem dos arquivos do gerador não é a ordem narrativa: identificamos
# cada estágio pelo CONTEÚDO (quanta construção existe), não pelo nome.
...
def processar(origem: Path, estacao: str, ordem: str | None) -> None:
    ...
    Image.fromarray(paleta[None, :, :], "RGB").save(dest / "paleta.png")
    for nome, arr in nativos:
        q = _quantizar(arr, paleta)
        Image.fromarray(q).save(dest / f"{nome}.png")
        grande = Image.fromarray(q).resize(
            (NATIVO[0] * AMPLIACAO, NATIVO[1] * AMPLIACAO), Image.NEAREST)
        grande.save(dest / f"{nome}_{AMPLIACAO}x.png")
        u = len(np.unique(q.reshape(-1, 3), axis=0))
        print(f"    {nome}: {u} cores")
    (dest / "meta.json").write_text(json.dumps(
        {"nativo": list(NATIVO), "paleta": len(paleta),
         "ampliacao": AMPLIACAO, "estagios": NOMES}, indent=1))
